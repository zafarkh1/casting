...
const GeneralFill: React.FC<GeneralFillProps> = ({ control, setValue }) => {
  ...
  const selectCity = useRef<HTMLSelectElement>(null);
  const selectProfession = useRef<HTMLSelectElement>(null);
  const { openSectionFill, toggleSectionFill } = useSectionFillStore();
  const pathname = usePathname();
  const { setProfessionalLevel, setSearchGender, setCity, mode } =
    useFilterCatalogue();
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <div className="mt-4 relative">
                      ...
                      <span
                        onClick={() => openSelect(selectProfession)}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <>
                    <Button
                      onClick={() => {
                        setProfessionalLevel("amateur");
                        field.onChange("amateur");
                      }}
                      className={`catalogueBtn transition-all duration-500 ${
                        field.value === "amateur"
                          ? "bg-primary text-white"
                    ...
                    <Button
                      onClick={() => {
                        setProfessionalLevel("professional");
